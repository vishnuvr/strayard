...
Start an StraYard all-in-one server

This command helps you launch an StraYard all-in-one server, which allows
you to run all of the components of an StraYard system on a server with Docker. Running

    $ strayard start

will start StraYard listening on all interfaces, launch an etcd server to store persistent
data, and launch the OpenShift system components. The server will run in the foreground until
you terminate the process.  This command delegates to "strayard start master" and 
"strayard start node".


Note: starting StraYard without passing the --master address will attempt to find the IP
address that will be visible inside running Docker containers. This is not always successful,
so if you have problems tell StraYard what public address it will be via --master=<ip>.
...
func startProfiler() {
	if cmdutil.Env("STRAYARD_PROFILE", "") == "web" {
		go func() {
